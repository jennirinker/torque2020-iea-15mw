...
from _inputs import (steady_dir, model_keys, i_gspd, i_pit, i_uhub, i_pow, i_flp,
                     i_tbfa, i_gtrq, i_edg, i_tipe, i_tipf, i_thr)
...
plot_keys = [('BldPitch1', i_pit, 1), ('GenSpeed', i_gspd, 1), ('GenPwr', i_pow, 1e-3) ,
             ...
             ('TwrBsMyt', i_tbfa, 1), ('TipDxb1', i_tipe, 1)]
# ('GenPwr', i_pow, 1e-3)  ('RtAeroFxh', i_thr, 1e3)
model_keys = [model_keys[0]]



# can plot up to 8
fig, axs = plt.subplots(2, 4, figsize=(12, 4), clear=True, num=1)
...
for i, (fastname, h2name) in enumerate(model_keys):
    ...
    for j, (fast_key, h2_chan, h2scl) in enumerate(plot_keys):
        if 'RootMy' in fast_key and 'BD' in fastname:
            fast_key = 'B1RootMyr'
            h2scl = -1e3
        elif 'RootMx' in fast_key and 'BD' in fastname:
            fast_key = 'B1RootMxr'
            h2scl = -1e3
